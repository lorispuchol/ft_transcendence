...
function ResponsiveAppBar() {
	...
	const pages = [['gaming', '/game'], ['everyone', '/everyone'], ['chat', '/chat']];
			  ...
			  <Container maxWidth="xl">
			    <Toolbar disableGutters>
			      ...
			      <Box sx={{ flexGrow: 1, display: { md: 'flex' } }}>
			        {pages.map((page) => (
			          <Button key={page[0]} onClick={() => redirection(page[1])} sx={{ my: 2, color: 'white', display: 'block' }}>
			            {page[0]}
			          </Button>
			        ))}
			      </Box>
			      <Box sx={{ flexGrow: 0 }}>
			        <Tooltip title="Open profile">
			          <IconButton onClick={() => redirection('/profile')} sx={{ p: 0 }}>
			            <Avatar alt="Lemy Sharp" src={defaultAvatar} />
			          </IconButton>
			        </Tooltip>
			      </Box>
			    </Toolbar>
